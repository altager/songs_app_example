...
from functests.test_utils.response_validators import (
    AvgDifficultyResponse,
    ErrorResponse,
)


def test_get_avg_difficulty_for_all_songs(
    cleanup_cache, create_document, cleanup_db, cfg
):
    create_document(
        {
            "artist": "Test artist",
...
def test_get_avg_difficulty_for_specific_level(
    cleanup_cache, create_document, cleanup_db, cfg
):
    create_document(
        {
    ...
    response = requests.get(
        cfg.URL_PREFIX + "/songs/avg/difficulty?" + urlencode({"level": 9})
    )
...
def test_get_avg_difficulty_not_existing_level(cfg):
    response = requests.get(
        cfg.URL_PREFIX + "/songs/avg/difficulty?" + urlencode({"level": 999})
    )
...
def test_get_avg_difficulty_invalid_level(level, cfg):
    response = requests.get(
        cfg.URL_PREFIX + "/songs/avg/difficulty?" + urlencode({"level": level})
    )
