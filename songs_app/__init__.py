...
def upload_json_data_from_file():
    data_path = os.path.join(
        os.path.abspath(os.path.dirname(__file__)), "sample_data/songs.json"
    )
...
def create_indexes():
    ...
    if "level" not in index_names:
        g.mongo_db.songs.create_index(
            "level", background=True, sparse=True, name="level"
        )
    if "artist_title_text" not in index_names:
        g.mongo_db.songs.create_index(
            [("artist", TEXT), ("title", TEXT)],
            background=True,
            name="artist_title_text",
        )
...
def configure_routes(app, cfg):
    ...
    app.add_url_rule(
        ...
        view_func=songs_handler.get_songs_list,
        methods=["GET"],
    )
    app.add_url_rule(
        "/songs/avg/difficulty",
        endpoint="get_avg_difficulty",
        view_func=songs_handler.get_avg_difficulty,
        methods=["GET"],
    )
    app.add_url_rule(
        "/songs/search",
        endpoint="search_songs",
        view_func=songs_handler.search_songs,
        methods=["GET"],
    )
    app.add_url_rule(
        "/songs/rating",
        endpoint="set_rating",
        view_func=songs_handler.set_rating,
        methods=["POST"],
    )
    app.add_url_rule(
        "/songs/avg/rating/<object_id:song_id>",
        endpoint="get_song_rating",
        view_func=songs_handler.get_song_rating,
        methods=["GET"],
    )
...
def get_config():
    return (
        app_config[os.getenv("SONGS_APP_CONFIG")]
        if os.getenv("SONGS_APP_CONFIG")
